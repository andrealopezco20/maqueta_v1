...
window.__MODULE_PATHS = {
  ...
  'electricidad-source': 'modulos/electricidad_al_toque/electricidad_al_toque.html?v=elec-tools-circle-v2',
...
(function initMasificacionReferenceModals(){
  ...
  function docDefaults(){
    ...
    return {
      ...
      articulo1:'Aprobar la Liquidacion Parcial FISE - Redes de la agrupación indicada, segun el monto consignado en el Informe Tecnico Legal correspondiente.',
  ...
  function exportDocPdf(){
    ...
    var d=readDoc(),pdf=new ctor({orientation:'landscape'}),rows=liqItems.map(function(it){return [it.codigoVNR,it.partida,it.cantidad+' '+it.unidad,money(it.costoUnitario,4),it.objetoGIS||'Sin vinculo',money(Number(it.cantidad||0)*Number(it.costoUnitario||0),2)];});
    pdf.setFontSize(16);pdf.text(activeDocView==='informe'?'Informe Tecnico VNR / Osinergmin':'Resolucion de Direccion Ejecutiva',14,16);pdf.setFontSize(10);pdf.text('Agrupación: '+(project().nombre||project().id||'Masificacion FISE'),14,24);pdf.text('Documento: '+(activeDocView==='informe'?d.numeroInforme:d.numeroResolucion),14,30);
    if(typeof pdf.autoTable==='function')pdf.autoTable({startY:36,head:[['Codigo VNR','Partida','Cantidad','Tarifa Osinergmin','Objeto GIS','Subtotal']],body:rows,styles:{fontSize:7},headStyles:{fillColor:[15,118,110]}});
