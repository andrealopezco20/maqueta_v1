...
function refreshActiveProjectPanel(){const p=currentProject();if(!p)return;const box=qs('#details');if(box)box.classList.remove('project-details-hidden');const bonogas=typeof isBonogasSatcontrolView==='function'&&isBonogasSatcontrolView();if(qs('#infoContextLabel'))qs('#infoContextLabel').textContent=bonogas?'Información seleccionada':'Proyecto activo:';if(qs('#infoName'))qs('#infoName').textContent=p.nombre;if(qs('#infoPlace'))qs('#infoPlace').textContent=p.distrito+' · '+p.provincia+' – '+p.departamento;renderProjectDetailsPanel(p);if(bonogas&&typeof window.renderBonogasProjectBeneficiaryTable==='function'){const rows=typeof window.getBonogasFilteredRows==='function'?window.getBonogasFilteredRows():window.currentSupplyRecords||[];window.renderBonogasProjectBeneficiaryTable(p,rows);}}
...
(function(){
  ...
  function renderBonogasProjectBeneficiaryTable(project,records){
    ...
    section.innerHTML='<h4>Beneficiarios del proyecto · '+rows.length+' visibles</h4><div class="bonogas-beneficiary-table-wrap"><table class="bonogas-beneficiary-table"><thead><tr><th>Suministro</th><th>Beneficiario</th>'+(hasDni?'<th>DNI</th>':'')+'<th>Estado</th></tr></thead><tbody>'
...
(function(){
  ...
  function ensureTheme(){const panel=document.getElementById('themesPanel');if(!panel||document.getElementById('bonogasMorosityTheme'))return;const card=document.createElement('div');card.id='bonogasMorosityTheme';card.className='bonogas-theme-card';card.innerHTML='<h4>Morosidad y recaudación</h4><p>Visualice los suministros con deuda según los filtros BonoGas aplicados.</p><div class="bonogas-theme-scale"></div><div class="bonogas-theme-labels"><span>Baja</span><span>Media</span><span>Crítica</span></div><button class="bonogas-theme-action" id="bonogasMorosityThemeBtn" type="button">Activar mapa de calor</button>';panel.appendChild(card)}
  function ensureLegend(){const shell=document.querySelector('.map-shell');if(!shell)return null;let legend=document.getElementById('bonogasMorosityLegend');if(!legend){legend=document.createElement('div');legend.id='bonogasMorosityLegend';legend.className='bonogas-morosity-legend';legend.innerHTML='<b>Morosidad por suministro</b><span><i style="background:#22c55e"></i>Baja · hasta S/ 300</span><span><i style="background:#facc15"></i>Media · S/ 301 a S/ 900</span><span><i style="background:#ef4444"></i>Crítica · más de S/ 900</span>';shell.appendChild(legend)}return legend}
  ...
  toggleHeat=function(){
    ...
    const btn=document.getElementById('bonogasMorosityThemeBtn'),legend=ensureLegend();
    if(heatLayer){
      ...
      if(btn){btn.textContent='Activar mapa de calor';btn.classList.remove('active');}
      if(typeof window.ensureBonogasMapLayer==='function')window.ensureBonogasMapLayer({zoomToResults:false});
      if(typeof showToast==='function')showToast('Mapa de morosidad desactivado');
      return;
    }
    ...
    const debts=rows.map((r,i)=>Number(r.montoPendiente!=null?r.montoPendiente:((i%5)*280)));
    const maxDebt=Math.max(1,...debts);
    const cells=new Map();
    rows.forEach((r,i)=>{const lat=Number(r.lat),lng=Number(r.lng),key=(Math.round(lat*5)/5).toFixed(1)+','+(Math.round(lng*5)/5).toFixed(1),prev=cells.get(key)||{lat:0,lng:0,debt:0,count:0};prev.lat+=lat;prev.lng+=lng;prev.debt+=debts[i];prev.count++;cells.set(key,prev);});
    const zones=Array.from(cells.values()).slice(0,100),maxZoneDebt=Math.max(1,...zones.map(cell=>cell.debt/Math.max(1,cell.count)));
    const heatPoints=zones.map(cell=>{const debt=cell.debt/Math.max(1,cell.count);return [cell.lat/cell.count,cell.lng/cell.count,Math.max(.14,Math.min(1,debt/maxZoneDebt))];});
    ...
    if(btn){btn.textContent='Desactivar mapa de calor';btn.classList.add('active');}
    if(typeof showToast==='function')showToast('Mapa de calor activado · '+source.length+' registros filtrados');
  };
