...
window.__FOTO_MODULE_VERSION = 'foto-peru-overview-v10';

window.__MODULE_PATHS = {
  'fotovoltaico-source': 'modulos/fotovoltaico/fotovoltaico.html',
  'electricidad-source': 'modulos/electricidad_al_toque/electricidad_al_toque.html',
  'vale-fise-source': 'modulos/vale_fise/vale_fise.html',
  'ahorro-gnv-source': 'modulos/ahorro_gnv/ahorro_gnv.html',
  'ahorro-gnv-satcontrol-source': 'modulos/ahorro_gnv/ahorro_gnv_satcontrol.html',
  'masificacion-source': 'modulos/masificacion/masificacion.html',
  'mcter-source': 'modulos/mtcer/mtcer.html'
};
...
async function loadShellModules() {
  ...
  var tasks = [];
  var adminVersion = 'proyectos-module-tools-hub-v2';
  ...
  if (bonogasHost && !bonogasHost.dataset.loaded) {
    tasks.push(fetch('modulos/bonogas/bonogas.html').then(function(r) {
      return r.text();
    }).then(function(html) {
      bonogasHost.innerHTML = html;
      bonogasHost.dataset.loaded = '1';
    }));
  }
...
const projects=[
  ...
  {id:'FISE-2026-002',nombre:'PROYECTO 2',lider:'Oliver Gonzales',responsables:['Operaciones','Validador FISE'],departamento:'Arequipa',provincia:'Arequipa',distrito:'Yanahuara',estado:'Aprobado',beneficiarios:64,area:'0.12 ha',tipo:'Masificación de gas FISE',lat:-16.3850,lng:-71.5380},
  {id:'FISE-2026-003',nombre:'PROYECTO 3',lider:'Oliver Gonzales',responsables:['Catastro','Campo'],departamento:'Arequipa',provincia:'Arequipa',distrito:'Cayma',estado:'En ejecución',beneficiarios:230,area:'0.48 ha',tipo:'Masificación de gas FISE',lat:-16.3760,lng:-71.5470},
  {id:'FISE-2026-004',nombre:'PROYECTO 4',lider:'Oliver Gonzales',responsables:['Instalaciones','Social'],departamento:'Arequipa',provincia:'Arequipa',distrito:'Yanahuara',estado:'En evaluación',beneficiarios:94,area:'0.18 ha',tipo:'Masificación de gas FISE',lat:-16.3910,lng:-71.5480},
  {id:'FISE-2026-005',nombre:'PROYECTO 5',lider:'Oliver Gonzales',responsables:['Campo','Técnico'],departamento:'Arequipa',provincia:'Arequipa',distrito:'Cerro Colorado',estado:'Aprobado',beneficiarios:156,area:'0.32 ha',tipo:'Masificación de gas FISE',lat:-16.3700,lng:-71.5600},
  {id:'FISE-2026-006',nombre:'PROYECTO 6',lider:'Oliver Gonzales',responsables:['Operaciones','GIS'],departamento:'Arequipa',provincia:'Arequipa',distrito:'Paucarpata',estado:'En ejecución',beneficiarios:312,area:'0.55 ha',tipo:'Masificación de gas FISE',lat:-16.4150,lng:-71.5000}
];
...
function clearProyectosForeignMapLayers(){if(window.bonogasHuLayer&&leafletMap&&leafletMap.hasLayer(window.bonogasHuLayer))leafletMap.removeLayer(window.bonogasHuLayer);window.bonogasHuLayer=null;const main=qs('.main');if(main){main.classList.remove('bonogas-satcontrol-mode','bonogas-active');}}
...
let selectedValidacionId=validaciones[0].id;let valLeafletMap=null;let valMorosityLayers=null;
...
function renderDonutChart(records,filterKey=currentStatsFilter,sourceLabel='Proyecto seleccionado'){const chart=qs('#donutChart');if(!chart)return;const data=getChartDataset(records,filterKey);const total=Math.max(data.leftValue+data.rightValue,1);const percent=Math.round((data.leftValue/total)*100);qs('#donutTitle').textContent='Estadística · '+data.title;qs('#donutSubtitle').textContent='Fuente: '+sourceLabel;qs('#donutCases').textContent=data.leftValue+data.rightValue;qs('#donutLegendA').textContent='■ '+data.leftLabel+' ('+data.leftValue+')';qs('#donutLegendB').textContent='■ '+data.rightLabel+' ('+data.rightValue+')';chart.style.background=`conic-gradient(#22d3ee 0 ${percent}%, #31c48d ${percent}% 100%)`;}
...
function downloadExcelFile(filename,html){const blob=new Blob(['﻿'+html],{type:'application/vnd.ms-excel;charset=utf-8;'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=filename;document.body.appendChild(a);a.click();document.body.removeChild(a);setTimeout(()=>URL.revokeObjectURL(url),800);}
...
function updateStatsByCurrentSelection(){const baseRecords=activeSelectionRecords.length?activeSelectionRecords:(activeAreaRecords.length?activeAreaRecords:currentSupplyRecords);const sourceLabel=activeSelectionRecords.length?'Objetos seleccionados':(activeAreaRecords.length?'Área seleccionada':'Proyecto seleccionado');renderDonutChart(baseRecords,currentStatsFilter,sourceLabel);}
...
function clearRightPanelForMapSelection(showMessage=true){ensureRightPanelVisible();closeMapPanels();if(drawLayer)drawLayer.clearLayers();drawingPoints=[];circleCenter=null;tempShape=null;activeAreaRecords=[];activeSelectionRecords=[];clearObjectSelection(false);selectedObjects=[];selectedLayerRefs.clear();if(qs('#infoContextLabel'))qs('#infoContextLabel').textContent='Selección:';if(qs('#infoName'))qs('#infoName').textContent='SELECCIONE EN EL MAPA';if(qs('#infoPlace'))qs('#infoPlace').textContent='Use dedo de selección, área o círculo';if(qs('#details'))qs('#details').innerHTML='';if(qs('#supplyDetails')){qs('#supplyDetails').className='supply-card empty';qs('#supplyDetails').innerHTML='<h3>Información de la selección</h3><p class="supply-hint">El panel está limpio. Use el dedo de selección para elegir una vivienda/lote, o dibuje un círculo o polígono para cargar la información del área seleccionada.</p>';}resetEvidencePanel();renderAreaStats(null);renderDonutChart([],currentStatsFilter,'Sin selección');renderMapTimeline(null);setMapTool('select');if(showMessage)showToast('Panel derecho limpio. Seleccione en el mapa para cargar información.');}
function updateRightPanelForObjectSelection(records,selectedRecord){ensureRightPanelVisible();const p=currentProject();const normalized=(records||[]).map(normalizeSelectedRecord);const stats=summarizeRecords(normalized);const d=normalizeSelectedRecord(selectedRecord||normalized[0]||{});if(qs('#infoContextLabel'))qs('#infoContextLabel').textContent='Selección:';if(normalized.length>1){if(qs('#infoName'))qs('#infoName').textContent='SELECCIÓN MÚLTIPLE';if(qs('#infoPlace'))qs('#infoPlace').textContent=normalized.length+' objetos seleccionados · Ctrl + clic';if(qs('#details'))qs('#details').innerHTML=`<p><span>Origen:</span> Dedo de selección</p><p><span>Área de selección:</span> Objetos individuales seleccionados</p><p><span>Registros seleccionados:</span> ${stats.total}</p><p><span>Beneficiarios:</span> ${stats.beneficiarios}</p><p><span>Liquidaciones:</span> ${formatMoney(stats.liquidacion)}</p><p><span>Total morosidad:</span> ${formatMoney(stats.pendiente||0)}</p>`;}else{if(qs('#infoName'))qs('#infoName').textContent=d.suministro||d.beneficiario||('LOTE · '+(d.estrato||'SELECCIONADO'));if(qs('#infoPlace'))qs('#infoPlace').textContent=(d.objectType||'Objeto seleccionado')+' · '+(p.provincia||'')+' – '+(p.departamento||'');if(qs('#details'))qs('#details').innerHTML=`<p><span>Origen:</span> Dedo de selección</p><p><span>Área de selección:</span> ${d.objectType==='Lote / área censal'?'Lote / área censal delimitada':'Objeto individual seleccionado'}</p><p><span>Beneficiarios:</span> ${d.beneficiarios||1}</p><p><span>Liquidación:</span> ${formatMoney(d.liquidacion||0)}</p><p><span>Morosidad:</span> ${formatMoney(d.montoPendiente||0)}</p>`;}renderProjectEvidences(p);}
...
function updateRightPanelForAreaSelection(records,title,extraInfo){ensureRightPanelVisible();const p=currentProject();const normalized=(records||[]).map(normalizeSelectedRecord);const stats=summarizeRecords(normalized);const areaLabel=extraInfo||'Área circunscrita por utilitario GIS';const label=qs('#infoContextLabel');if(label)label.textContent='Selección:';if(qs('#infoName'))qs('#infoName').textContent=title||'ÁREA SELECCIONADA';if(qs('#infoPlace'))qs('#infoPlace').textContent=areaLabel;if(qs('#details'))qs('#details').innerHTML=`<p><span>Origen:</span> Utilitario GIS</p><p><span>Área de selección:</span> ${areaLabel}</p><p><span>Registros encontrados:</span> ${stats.total}</p><p><span>Beneficiarios:</span> ${stats.beneficiarios}</p><p><span>Liquidaciones:</span> ${formatMoney(stats.liquidacion)}</p><p><span>Total morosidad:</span> ${formatMoney(stats.pendiente||0)}</p>`;renderProjectEvidences(p);renderAreaSelectionDetails(normalized,title);}
...
document.addEventListener('click',e=>{if(e.target&&e.target.id==='openFullInstallerRankingBtn'){openInstallerRanking(qs('#valInstallerMiniDetail h4')?.textContent||'Instalaciones del Norte S.A.C.')}});
...
function appendFloatingChatMessage(role,text){
...
function sendFloatingChatMessage(text){
  ...
  setTimeout(()=>appendFloatingChatMessage('bot','Claro, puedo ayudarte con eso desde SATCONTROL.'),220);
}
...
];window.paymentRows=paymentRows;const filtered=paymentRows.filter(r=>(!empresa||r.empresa===empresa)&&(!estado||r.estado===estado)&&(!fecha||r.fechaIso===fecha)&&(!distrito||r.distrito===distrito)&&(!q||normalizeText(r.id+' '+r.beneficiario+' '+r.empresa+' '+r.distrito).includes(q)));window.filteredPaymentRows=filtered;renderPaymentZones(filtered);const body=qs('#validacionesTableBody');if(body){
  /* Enriquecer con datos BONOGAS por beneficiario */
  function findBonogasForVal(beneficiario){var nrm=typeof normalizeText==='function'?normalizeText:function(v){return String(v||'').toLowerCase();};var en=(window._enrichedBeneficiarios||[]).find(function(d){return nrm(d.beneficiario||'')===nrm(beneficiario||'');});if(!en&&typeof bonogasMapRows==='function'){en=bonogasMapRows().find(function(d){return nrm(d.beneficiario||'')===nrm(beneficiario||'');});}return en;}
  body.innerHTML=filtered.map(r=>{
    const zoneTotal=filtered.filter(x=>x.distrito===r.distrito).reduce((s,x)=>s+Number(x.monto||0),0);
    const zoneLevel=zoneTotal>=1550?'critical':zoneTotal>=1450?'high':'medium';
    const zoneBadge=zoneLevel==='critical'?'Zona roja':'Zona alta';
    const bdata=findBonogasForVal(r.beneficiario)||{};
    const sumStr=bdata.suministro?`<small style="display:block;color:#67e8f9;font-weight:900;margin-top:2px">${bdata.suministro}</small>`:'';
    const estStr=bdata.estrato?`<small style="display:block;color:#94a3b8;font-weight:800;margin-top:1px">Est. ${bdata.estrato}</small>`:'';
    return `<tr>`
      +`<td><input type="checkbox" aria-label="Seleccionar expediente"></td>`
      +`<td><b>${r.id}</b></td>`
      +`<td>${r.beneficiario}${sumStr}${estStr}</td>`
      +`<td><button class="installer-link" type="button" data-installer-name="${r.empresa}" title="Ver ranking e histórico"><span class="rank-mini"><svg class="svg-icon" aria-hidden="true"><use href="#i-star"></use></svg></span>${r.empresa}</button></td>`
      +`<td><span class="pay-status ${zoneLevel==='critical'?'obs':'pending'}">${r.distrito}</span><small style="display:block;color:${zoneLevel==='critical'?'#fecaca':'#fde68a'};font-weight:900;margin-top:4px">${zoneBadge} · ${r.manzana}</small></td>`
      +`<td>${r.fecha}</td>`
      +`<td>${formatMoney(r.monto)}</td>`
      +`<td><span class="pay-status ${r.chip}">${r.estado}</span></td>`
      +`<td><button class="sol-action-btn" type="button" data-val-id="${r.id}">⋮</button></td>`
      +`</tr>`;
  }).join('')||'<tr><td colspan="9">No se encontraron expedientes.</td></tr>';}qs('#valPaymentCount')&&(qs('#valPaymentCount').textContent=filtered.length);qsa('[data-val-id]').forEach(btn=>btn.addEventListener('click',()=>selectValidacion(btn.dataset.valId)));qsa('[data-installer-name]').forEach(btn=>btn.addEventListener('click',e=>{e.stopPropagation();openInstallerRanking(btn.dataset.installerName)}));selectValidacion(selectedValidacionId&&String(selectedValidacionId).startsWith('FISE-')?selectedValidacionId:'FISE-2025-0002489',false)}
...
function renderInstallerInline(selectedName){const list=qs('#valInstallerMiniList'),detail=qs('#valInstallerMiniDetail');if(!list||!detail||!window.installerRankingData)return;const sorted=[...installerRankingData].sort((a,b)=>b.score-a.score).slice(0,6);const selected=installerRankingData.find(x=>x.name===selectedName)||sorted[0];list.innerHTML=sorted.map((item,idx)=>`<button class="val-installer-mini-card ${item.name===selected.name?'selected':''}" type="button" data-mini-installer="${item.name}"><b>#${idx+1} ${item.name}</b><span>${item.expedientes} expedientes · ${item.conformidad}% conforme</span><small class="${installerStatusClass(item)}">${item.aptitud} · ${item.score}/100</small></button>`).join('');qsa('[data-mini-installer]').forEach(btn=>btn.addEventListener('click',()=>renderInstallerInline(btn.dataset.miniInstaller)));const recClass=selected.aptitud==='Apta'?'':selected.aptitud==='Observada'?'warn':'bad';detail.innerHTML=`<h4>${selected.name}</h4><div class="mini-score-row"><span>Score actual</span><b>${selected.score}/100</b></div><div class="mini-score-track"><div class="mini-score-fill" style="width:${selected.score}%"></div></div><div class="mini-score-row"><span>Histórico trimestral</span><b>${selected.historico.join(' · ')}</b></div><div class="mini-score-row"><span>Retrabajos</span><b>${selected.retrabajos}%</b></div><div class="mini-score-row"><span>Plazo promedio</span><b>${selected.plazo} días</b></div><div class="mini-recommendation ${recClass}">${selected.recomendacion}</div>`;}
...
function openValidacionesEnvironment(){const main=qs('.main');main?.classList.add('validations-mode');main?.classList.remove('requests-mode','vale-fise-mode','ahorro-gnv-mode','fotovoltaico-mode','electricidad-mode','masificacion-mode','hospital-mode','create-project-mode','project-list-mode','project-delete-mode','spa-mode','bonogas-active','bonogas-satcontrol-mode');qs('#navValidaciones')?.classList.add('active');qs('#navSolicitudes')?.classList.remove('active');qs('#navDashboard')?.classList.remove('active');qs('#navBonoSatcontrol')?.classList.remove('active');qs('#navValeFise')?.classList.remove('active');qs('#navAhorroGnv')?.classList.remove('active');qs('#navFotovoltaico')?.classList.remove('active');qs('#navElectricidad')?.classList.remove('active');qs('#navMasificacionSatcontrol')?.classList.remove('active');const title=qs('.topbar h1'),sub=qs('.topbar p');if(title)title.textContent='SATCONTROL · VALIDACIONES';if(sub){sub.textContent='Proceso No Concesionaria / Instaladora · Expedientes, Declaraciones Juradas y trazabilidad';sub.style.display='';}renderValidaciones();renderInstallerInline('Instalaciones del Norte S.A.C.');initValLeafletMap();resizeMapAfterLayout();showToast('Validaciones')}
function closeSolicitudesEnvironment(){const main=qs('.main');main?.classList.remove('requests-mode','validations-mode','vale-fise-mode','ahorro-gnv-mode','fotovoltaico-mode','electricidad-mode','masificacion-mode','hospital-mode','create-project-mode','project-list-mode','project-delete-mode');qs('#navSolicitudes')?.classList.remove('active');qs('#navValidaciones')?.classList.remove('active');qs('#navValeFise')?.classList.remove('active');qs('#navAhorroGnv')?.classList.remove('active');qs('#navFotovoltaico')?.classList.remove('active');qs('#navElectricidad')?.classList.remove('active');qs('#navMasificacionSatcontrol')?.classList.remove('active');const title=qs('.topbar h1'),sub=qs('.topbar p');if(title)title.textContent='SATCONTROL PROYECTOS';if(sub){sub.textContent='Gestión territorial, documentos, liquidaciones y seguimiento operativo';sub.style.display='';}resizeMapAfterLayout();showToast('SATCONTROL PROYECTOS')}
...
function updateOverlayVisibility(){if(!leafletMap)return;const main=qs('.main');if(main&&(main.classList.contains('bonogas-satcontrol-mode')||main.classList.contains('bonogas-active'))){syncUploadedMapLayersToLeaflet();return;}[coberturaLayer,troncalLayer,ramalesLayer,concesionariaLayer,districtLayer,selectedDistrictLayer,morosityLayer,estratoLayer,beneficiaryLayer].forEach(layer=>{if(layer&&leafletMap.hasLayer(layer))leafletMap.removeLayer(layer)});const coberturaOn=qs('#layerCobertura')?.checked??true;const troncalOn=qs('#layerTroncal')?.checked??true;const ramalesOn=qs('#layerRamales')?.checked??true;const concesionariaOn=qs('#layerConcesionaria')?.checked??true;const distritosOn=qs('#layerDistritos')?.checked??false;const morosidadOn=qs('#layerMorosidad')?.checked??false;const estratoOn=qs('#layerEstrato')?.checked??true;const beneficiaryStates=getBeneficiaryLayerStates();const beneficiariesOn=beneficiaryStates.liquidados||beneficiaryStates.pendientes||beneficiaryStates.dentro||beneficiaryStates.fuera;const p=currentProject();if(coberturaLayer&&coberturaOn)coberturaLayer.addTo(leafletMap);if(troncalLayer&&troncalOn)troncalLayer.addTo(leafletMap);if(ramalesLayer&&ramalesOn)ramalesLayer.addTo(leafletMap);if(concesionariaLayer&&concesionariaOn)concesionariaLayer.addTo(leafletMap);if(districtLayer&&distritosOn)districtLayer.addTo(leafletMap);if(selectedDistrictLayer&&selectedDistrictId&&distritosOn)selectedDistrictLayer.addTo(leafletMap);if(estratoLayer&&estratoOn)estratoLayer.addTo(leafletMap);if(beneficiariesOn&&p){const benCount=Math.min(38,Math.max(16,Math.round((p.beneficiarios||24)/6)));if(!beneficiaryLayer||lastSyncedProjectId!==p.id){beneficiaryLayer=buildBeneficiaries(p.lat,p.lng,benCount,p);morosityLayer=buildMorosityHeatLayer(p.lat,p.lng);lastSyncedProjectId=p.id;}if(beneficiaryLayer)beneficiaryLayer.addTo(leafletMap);}if(morosityLayer&&morosidadOn)morosityLayer.addTo(leafletMap);qs('#morosityLegend')?.classList.toggle('open',!!morosidadOn);syncUploadedMapLayersToLeaflet();if(projectMarker&&p){projectMarker.setLatLng([p.lat,p.lng]);if(!leafletMap.hasLayer(projectMarker))projectMarker.addTo(leafletMap);}updateActiveLayerLabel();}
...
function exportValidationByFormat(format){const rows=window.filteredPaymentRows||window.paymentRows||[];const empresa=qs('#valEmpresaFilter')?.value||'Todas';const estado=qs('#valEstadoFilter')?.value||'Todos';const fecha=qs('#valFechaFilter')?.value||'Todas';const distrito=qs('#valDistritoFilter')?.value||'Todos';if(format==='xlsx')exportFilteredValidaciones(rows,true);showToast(`Reporte ${format.toUpperCase()} descargado con filtros: ${empresa} / ${estado} / ${fecha} / ${distrito}`);toggleValidationExportMenu(false);}
...
async function loadIntegratedModuleSources(){
  ...
  function getIntegratedSatcontrolLayoutCss(){
    return getIntegratedSatPanelToggleBaseCss()+getIntegratedPanelCollapseCss()+getIntegratedNativeToggleHideCss()+
      ...
      'grid-template-columns:320px minmax(480px,1fr) 380px!important;gap:18px!important;padding:18px!important;overflow:hidden!important}'+
      '.content.left-hidden{grid-template-columns:0 minmax(480px,1fr) 380px!important}'+
      '.content.right-hidden{grid-template-columns:320px minmax(480px,1fr) 0!important}'+
      '.content.left-hidden.right-hidden{grid-template-columns:0 minmax(480px,1fr) 0!important}'+
      ...
      '.map-wrap,.map-shell{min-height:0!important;height:100%!important}'+
      '.map-shell{border-radius:24px!important}'+
      '.left-panel,.info-panel,.right-panel,.card.left-panel,.card.right-panel{border-radius:22px!important;min-height:0!important}'+
      '.masif-satcontrol-section{padding:18px!important;height:100vh!important;min-height:0!important;box-sizing:border-box!important;overflow:hidden!important}'+
      ...
      '.masif-satcontrol-grid.right-collapsed{grid-template-columns:320px minmax(480px,1fr) 0!important}'+
      '.masif-satcontrol-grid.left-collapsed{grid-template-columns:0 minmax(480px,1fr) 380px!important}'+
      '.masif-satcontrol-grid.left-collapsed.right-collapsed{grid-template-columns:0 minmax(480px,1fr) 0!important}'+
      ...
      '.masif-map-shell,.masif-instant-map{min-height:0!important;height:100%!important;border-radius:24px!important}'+
      ...
      '@media(max-width:1180px){.content{grid-template-columns:320px 1fr!important}.info-panel,.right-panel{display:none!important}.sat-right-toggle{right:12px!important}}';
  }
  ...
  function injectSatPanelToggleCss(html){
    ...
    const panelScript=isValeFise||isFoto?'':'<script id="integrated-sat-panel-toggle-script">(function(){function refreshMap(){setTimeout(function(){try{if(window.leafletMap&&window.leafletMap.invalidateSize)window.leafletMap.invalidateSize();if(window.__fotoMap&&window.__fotoMap.invalidateSize)window.__fotoMap.invalidateSize();if(window.map&&window.map.invalidateSize)window.map.invalidateSize();}catch(e){}},260)}function bindBtn(el,fn){if(!el||el.dataset.innerBound)return;el.dataset.innerBound="1";el.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();fn()})}function bind(){var content=document.querySelector(".content");if(!content)return;window.toggleProjects=function(){content.classList.toggle("left-hidden");refreshMap()};window.toggleInfo=function(){content.classList.toggle("right-hidden");refreshMap()};bindBtn(document.getElementById("toggleProjectsMapBtn"),window.toggleProjects);bindBtn(document.getElementById("toggleInfoMapBtn"),window.toggleInfo)}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",bind);else bind();window.addEventListener("load",bind);setTimeout(bind,120);setTimeout(bind,480);})();<\/script>';
  ...
  function wireIntegratedFramePanelToggles(frame){
    ...
    try{
      ...
      const refresh=()=>setTimeout(()=>{try{win.leafletMap?.invalidateSize?.();win.__fotoMap?.invalidateSize?.();win.map?.invalidateSize?.();}catch(err){}},260);
  ...
  function refreshGnvFrameMap(frame){
    ...
    const refresh=()=>{
      ...
      try{
        const doc=frame.contentDocument;
        const left=doc?.querySelector('#toggleProjectsMapBtn');
        const right=doc?.querySelector('#toggleInfoMapBtn');
        if(left&&!left.dataset.bound){left.dataset.bound='true';left.addEventListener('click',()=>win.toggleProjects?.());}
        if(right&&!right.dataset.bound){right.dataset.bound='true';right.addEventListener('click',()=>win.toggleInfo?.());}
  ...
  if(elecFrame&&elecFrame.dataset.loaded!=='elec-peru-overview-v8'&&getModuleSource('electricidad-source')){
    elecFrame.srcdoc=injectSatPanelToggleCss(getModuleSource('electricidad-source'));
    elecFrame.dataset.loaded='elec-peru-overview-v8';
    injectTypographyIntoFrame(elecFrame);
  }
  if(masFrame&&masFrame.dataset.loaded!=='revert-v12:SATCONTROL'&&getModuleSource('masificacion-source')){
    masFrame.srcdoc=injectSatPanelToggleCss(getModuleSource('masificacion-source'));
    masFrame.dataset.loaded='revert-v12:SATCONTROL';
    injectTypographyIntoFrame(masFrame);
  }
  if(mcterFrame&&mcterFrame.dataset.loaded!=='mcter-peru-overview-v8'&&getModuleSource('mcter-source')){
    mcterFrame.srcdoc=injectSatPanelToggleCss(getModuleSource('mcter-source'));
    mcterFrame.dataset.loaded='mcter-peru-overview-v8';
...
(() => {
  ...
  function bindNav() {
    ...
    $('#navBonoSatcontrol')?.addEventListener('click', e => {
      e.preventDefault();
      e.stopImmediatePropagation();
      /* Activación directa — sin pasar por openSatcontrolView para evitar flash */
      const main = $('.main');
      if(main) main.classList.remove('requests-mode','validations-mode','hospital-mode','vale-fise-mode','ahorro-gnv-mode','fotovoltaico-mode','electricidad-mode','masificacion-mode','create-project-mode','project-list-mode','project-delete-mode','spa-mode');
      const content = $('.content');
      if(content) content.classList.remove('left-hidden','right-hidden');
      if(typeof alignSatcontrol === 'function') alignSatcontrol();
...
(function(){
  ...
  huStyle.textContent=[
    ...
    '.main.bonogas-satcontrol-mode #details{display:none!important}',
  ...
  function bonogasEstratoOptionsHtml(selected){
    let html='<option value="">Todos</option>';
    Object.keys(BONOGAS_ESTRATO_LABELS).forEach(function(k){
  ...
  function normalizeBonogasSatFilters(f){
    f=Object.assign({estrato:'',empresaInstaladora:'',concesionaria:'',fechaDesde:'',fechaHasta:''},f||{});
  ...
  function getBonogasFilteredRows(){
    return bonogasMapRows().filter(bonogasRecordMatchesFilters);
  }
  ...
  function bonogasMapRows(){
    ...
    const cities=[
      ...
      ['Chiclayo',-6.7714,-79.8409,'RedGas Contratistas','Cálidda'],
      ['Trujillo',-8.1084,-79.0215,'NorteGas SAC','Cálidda'],
  ...
  function updateBonogasLayerLabel(rows){
    ...
    label.innerHTML='Capa activa: <span style="color:#67e8f9">'+(geo?'Geoportal BONOGAS':'Suministros BONOGAS')+'</span> · '+esc(parts.join(' · '))+' · '+rows.length+' reg.';
  }
  ...
  function getBonogasFilterOptions(){
    const rows=bonogasMapRows();
    const empresas=Array.from(new Set(rows.map(function(r){return r.empresaInstaladora||'Sin empresa'}))).sort();
    const concesionarias=Array.from(new Set(rows.map(function(r){return r.concesionaria}).filter(Boolean))).sort();
    const fechas=Array.from(new Set(rows.map(bonogasRecordFechaKey).filter(Boolean))).sort();
    return {empresas:empresas,concesionarias:concesionarias,fechas:fechas};
  }
  ...
  function renderBonogasFiltersPanel(){
    ...
    const f=normalizeBonogasSatFilters(window.bonogasSatFilters||{});
    const opts=getBonogasFilterOptions();
    ...
    const hastaEl=$('#bonogasFilterFechaHasta');
    if(estratoSel){
      estratoSel.innerHTML=bonogasEstratoOptionsHtml(f.estrato||'');
    ...
    if(!wrap.dataset.bound){
      ...
      wrap.addEventListener('change',function(ev){
        ...
        if(id==='bonogasFilterEstrato'||id==='bonogasFilterEmpresa'||id==='bonogasFilterConcesionaria'||id==='bonogasFilterFechaDesde'||id==='bonogasFilterFechaHasta'){
          applyBonogasFilters();
        }
      });
    }
  ...
  function updateBonogasAreaSelectionPanel(records,title,extraInfo,stats){
    ...
    const infoCtx=$('#infoContextLabel');
    const infoName=$('#infoName');
    const infoPlace=$('#infoPlace');
    if(infoCtx)infoCtx.textContent='Seleccion GIS:';
    if(infoName)infoName.textContent=title||'Area seleccionada';
    if(infoPlace)infoPlace.textContent=areaLabel;
    const box=$('#supplyDetails');
    if(!box)return;
    if(!records.length){
      box.className='supply-card empty';
  ...
  function patchBonogasAreaSelection(){
    ...
    if(typeof clearDrawings==='function'&&!clearDrawings.bonogasPatched){
      ...
      clearDrawings=function(){
        ...
        const m=$('.main');
        if(m&&m.classList.contains('bonogas-satcontrol-mode'))updateBonogasCorrectionCard(getBonogasFilteredRows(),null);
      };
  ...
  function updateBonogasCorrectionCard(rows,selectionLabel){
    ...
    const late=rows.filter(bonogasIsLate);
    const byCompany={};
    late.forEach(function(r){const k=r.empresaInstaladora||'Sin empresa';byCompany[k]=(byCompany[k]||0)+1});
    const companyRows=Object.keys(byCompany).map(function(k){return '<tr><td>'+esc(k)+'</td><td>'+byCompany[k]+'</td><td><span class="hu-pill bad">Generar reporte</span></td></tr>'}).join('');
    const areaNote=selectionLabel&&rows.length
      ? esc(selectionLabel)+' · '+rows.length+' suministros en el area.'
      : 'Reporte de suministros sin instalacion interna finalizada en plazo mayor o igual a 90 dias, agrupado por empresa instaladora.';
    card.innerHTML='<div class="correction-head"><div><h3>Control de plazos Art. 25.9</h3><p>'+areaNote+'</p></div><button class="plazo-art259-info-btn" id="plazoArt259InfoBtn" type="button" title="Ver reporte de suministros" aria-label="Ver reporte de suministros"><svg class="svg-icon" aria-hidden="true"><use href="#i-search"></use></svg></button></div><div class="kpi-strip"><div class="mini-kpi"><span>Fuera de plazo</span><b>'+stats.penalizados+'</b><small>>= 90 dias calendario</small></div><div class="mini-kpi"><span>Dentro de plazo</span><b>'+stats.dentroPlazo+'</b><small>En construccion</small></div><div class="mini-kpi"><span>Habilitados</span><b>'+stats.conectados+'</b><small>Solo referencia estadistica</small></div></div><div class="hu-deadline-card"><h3>Empresas instaladoras con incumplimiento</h3><table class="hu-deadline-table"><thead><tr><th>Empresa</th><th>Suministros</th><th>Accion</th></tr></thead><tbody>'+(companyRows||'<tr><td colspan="3">Sin incumplimientos en los datos actuales.</td></tr>')+'</tbody></table></div>';
  ...
  function ensureBonogasMapLayer(){
    ...
    rows.forEach(function(r){
      const status=bonogasStatus(r);
      const color=status==='Fuera de plazo'?'#ef4444':status==='Dentro de plazo'?'#0ea5e9':status==='Pendiente liquidacion'?'#f59e0b':'#22c55e';
      const estratoLabel=r.estratoLabel||bonogasEstratoLabel(normalizeBonogasEstratoNum(r,r.idx!=null?Number(r.idx)-1:0));
      const marker=L.circleMarker([Number(r.lat),Number(r.lng)],{radius:7,fillColor:color,color:'#fff',weight:2,opacity:1,fillOpacity:.92});
    ...
    if(!hasVisibleUpload&&rows.length>1){
      try{leafletMap.fitBounds(rows.map(function(r){return [Number(r.lat),Number(r.lng)]}),{padding:[44,44]});}catch(e){}
    }
  }
  ...
  function alignSatcontrol(){
    ...
    if(!geoPortalMapActive())setTimeout(ensureBonogasMapLayer,140);
    ...
    const infoName=$('#infoName');
    const infoPlace=$('#infoPlace');
    if(infoCtx)infoCtx.textContent='Suministros';
    if(infoName)infoName.textContent='SATCONTROL · BONO GAS';
    if(infoPlace)infoPlace.textContent='Seleccione un registro en el mapa';
    renderBonogasFiltersPanel();
    currentSupplyRecords=getBonogasFilteredRows().slice();
    window.currentSupplyRecords=currentSupplyRecords.slice();
    updateBonogasCorrectionCard();
    if(typeof renderDonutChart==='function'){
      renderDonutChart(bonogasStatsBaseRows(),'estratoSocioeconomico','Filtros activos');
    }
    applyBonogasFilters();
  ...
  const djRows=[
    {id:'DJ-2026-0001',beneficiario:'Maria Quispe Huaman',dni:'43567891',suministro:'5208079',instalacion:'INS-5208079',direccion:'Calle Los Sauces 120',empresa:'Instalaciones del Norte S.A.C.',ruc:'20548796321',representante:'Carlos Perez Salas',firma:'Detectada',fotos:'5/6',resultado:'Observada',estado:'Rechazada',alertas:['Foto de gabinete borrosa','Direccion incompleta segun BonoGas 2.0']},
    {id:'DJ-2026-0002',beneficiario:'Jose Mamani Flores',dni:'40221876',suministro:'913777',instalacion:'INS-913777',direccion:'Mz. B Lt. 08',empresa:'Gas & Hogar E.I.R.L.',ruc:'20604588911',representante:'Rosa Diaz Leon',firma:'No detectada',fotos:'6/6',resultado:'Observada',estado:'Subsanada',alertas:['Firma del representante legal faltante']},
    {id:'DJ-2026-0003',beneficiario:'Rosa Condori Arias',dni:'46123988',suministro:'5410777',instalacion:'INS-5410777',direccion:'Jr. Amazonas 315',empresa:'Instalagas Peru S.A.C.',ruc:'20445566771',representante:'Luis Torres Vega',firma:'Detectada',fotos:'6/6',resultado:'Conforme',estado:'Aprobada',alertas:[]}
  ];
  let selectedDjId=djRows[0].id;
  const huAiUploads={dj:[],photos:[]};
  const huAiSteps={dj:'upload',photos:'upload'};
  const projectSatOriginal={};

  function getFilteredDjRows(){
    const q=norm($('#valSearch')&&$('#valSearch').value);
    const empresa=($('#valEmpresaFilter')&&$('#valEmpresaFilter').value)||'';
    const estado=($('#valEstadoFilter')&&$('#valEstadoFilter').value)||'';
    return djRows.filter(function(r){
      return (!empresa||r.empresa===empresa)&&(!estado||r.estado===estado)&&(!q||norm(r.id+' '+r.beneficiario+' '+r.empresa+' '+r.suministro).includes(q));
    });
  }
  ...
  function exportFilteredDjValidaciones(rows,format,quiet){
    ...
    const empresa=($('#valEmpresaFilter')&&$('#valEmpresaFilter').value)||'Todas';
    const estado=($('#valEstadoFilter')&&$('#valEstadoFilter').value)||'Todos';
  ...
  function patchDjValidacionesExport(){
    ...
    window.exportFilteredValidaciones=function(rows,quiet){
      const useRows=(rows&&rows.length)?rows:(window.filteredDjRows||getFilteredDjRows());
      if(useRows.length&&(useRows[0].resultado!=null||String(useRows[0].id||'').indexOf('DJ-')===0)){
        exportFilteredDjValidaciones(useRows,'xlsx',quiet);
        return;
      }
      if(typeof window._origExportFilteredValidaciones==='function')window._origExportFilteredValidaciones(rows,quiet);
    };
    if(typeof confirmExport==='function'&&!window.confirmExport.djPatched){
      ...
      window.confirmExport=function(){
        ...
        const isValidations=main&&main.classList.contains('validations-mode');
        const djRowsFiltered=window.filteredDjRows||getFilteredDjRows();
        if(isValidations&&djRowsFiltered.length){
  ...
  window.renderValidaciones=function(){
    const rows=getFilteredDjRows();
    window.filteredDjRows=rows;
    window.paymentRows=djRows;
    window.filteredPaymentRows=rows;
    const q=norm($('#valSearch')&&$('#valSearch').value),empresa=($('#valEmpresaFilter')&&$('#valEmpresaFilter').value)||'',estado=($('#valEstadoFilter')&&$('#valEstadoFilter').value)||'';
    const h=$('.val-table-head h3');if(h)h.innerHTML='Declaraciones Juradas en validacion IA (<span id="valPaymentCount">'+rows.length+'</span>)';
    const k=$$('.val-admin-kpi');if(k[0])k[0].querySelector('span').textContent='DJ conformes';if(k[0])k[0].querySelector('b').textContent=djRows.filter(r=>r.estado==='Aprobada').length;
    if(k[1])k[1].querySelector('span').textContent='En revision IA';if(k[1])k[1].querySelector('b').textContent=djRows.filter(r=>r.estado==='Rechazada').length;
    if(k[2])k[2].querySelector('span').textContent='Con alertas';if(k[2])k[2].querySelector('b').textContent=djRows.filter(r=>r.alertas.length).length;
    if(k[3])k[3].querySelector('span').textContent='Subsanadas';if(k[3])k[3].querySelector('b').textContent=djRows.filter(r=>r.estado==='Subsanada').length;
    const state=$('#valEstadoFilter');if(state)state.innerHTML='<option value="">Estado IA</option><option>Aprobada</option><option>Rechazada</option><option>Subsanada</option>';
    const empFilter=$('#valEmpresaFilter');if(empFilter){const oldEmp=empFilter.value;const empresas=Array.from(new Set(djRows.map(function(r){return r.empresa}))).sort();empFilter.innerHTML='<option value="">Todas las empresas</option>'+empresas.map(function(e){return '<option>'+esc(e)+'</option>'}).join('');if(oldEmp)empFilter.value=oldEmp;}
    const table=$('.val-admin-table');if(table&&table.tHead)table.tHead.innerHTML='<tr><th></th><th>DJ</th><th>Beneficiario</th><th>Empresa instaladora</th><th>Suministro</th><th>Fotos DJ</th><th>Resultado IA</th><th>Estado</th><th></th></tr>';
    const body=$('#validacionesTableBody');if(body)body.innerHTML=rows.map(function(r){return '<tr><td><input type="checkbox"></td><td><b>'+esc(r.id)+'</b></td><td>'+esc(r.beneficiario)+'</td><td><button class="installer-link" type="button" data-installer-name="'+esc(r.empresa)+'" title="Ver ranking e histórico" onclick="if(typeof openInstallerRanking===\'function\')openInstallerRanking(this.dataset.installerName)"><span class="rank-mini">★</span>'+esc(r.empresa)+'</button></td><td>'+esc(r.suministro)+'</td><td>'+esc(r.fotos)+'</td><td><span class="hu-pill '+(r.alertas.length?'warn':'ok')+'">'+esc(r.resultado)+'</span></td><td><span class="hu-pill '+(r.estado==='Aprobada'?'ok':r.estado==='Subsanada'?'warn':'bad')+'">'+esc(r.estado)+'</span></td><td><button class="sol-action-btn" type="button" data-dj-id="'+esc(r.id)+'">Ver</button></td></tr>'}).join('')||'<tr><td colspan="9">No se encontraron DJ.</td></tr>';
    $$('[data-dj-id]').forEach(function(b){b.addEventListener('click',function(){selectValidacion(b.dataset.djId)})});
    selectValidacion(selectedDjId,false);
  };

  window.selectValidacion=function(id,toast){
    const r=djRows.find(function(x){return x.id===id})||djRows[0];selectedDjId=r.id;
    const score=$('#validationScorecard');if(score){score.classList.add('compact-flow');/* score.innerHTML='<div class="ai-section-title"><div><h3>Validacion IA de DJ y fotografias</h3><p>Flujo unificado: subir evidencias, analizar con IA y revisar resultados por expediente.</p></div><span class="ai-badge">'+esc(r.estado)+'</span></div><div class="hu-ai-combined">'+renderHuAiFlow('dj','Validacion Inteligente de DJ','Cargue la Declaracion Jurada de 2 hojas y documentos del expediente.',r)+renderHuAiFlow('photos','Fotografias obligatorias de la DJ','Fachada, gabinete, ambiente y gasodomestico.',r)+'</div>';bindHuAiFlow(r); */}
    const dj=$('#djAiCard');if(dj){dj.classList.add('compact-flow');dj.innerHTML='<div class="ai-section-title"><div><h3>Resultado consolidado IA</h3><p>Resumen de alertas y criterios obligatorios para revision humana.</p></div><span class="ai-badge">HU IA</span></div><div class="dj-ai-grid"><div class="dj-ai-criterion done"><i>✓</i><div><b>Empresa instaladora</b><span>'+esc(r.empresa)+' / RUC '+esc(r.ruc)+'</span></div><small>Validada</small></div><div class="dj-ai-criterion done"><i>✓</i><div><b>Beneficiario</b><span>'+esc(r.beneficiario)+' / '+esc(r.dni)+'</span></div><small>Validado</small></div><div class="dj-ai-criterion '+(r.alertas.some(a=>/Direccion|direcci/i.test(a))?'warn':'done')+'"><i>!</i><div><b>Suministro y direccion</b><span>'+esc(r.suministro)+' / '+esc(r.direccion)+'</span></div><small>'+(r.alertas.some(a=>/Direccion|direcci/i.test(a))?'Alerta':'Validado')+'</small></div><div class="dj-ai-criterion '+(r.alertas.some(a=>/gabinete|borrosa/i.test(a))?'warn':'done')+'"><i>!</i><div><b>Fotografias</b><span>Fachada, gabinete, ambiente, gasodomestico</span></div><small>'+(r.alertas.some(a=>/gabinete|borrosa/i.test(a))?'Observada':'OK')+'</small></div></div><div class="dj-ai-alerts">'+(r.alertas.length?r.alertas.map(a=>'<div class="dj-ai-alert">'+esc(a)+'</div>').join(''):'<div class="dj-ai-alert ok">Sin alertas criticas. DJ apta para revision humana previa a liquidacion.</div>')+'</div>';}
    const detail=$('#valDetailCard');if(detail)detail.innerHTML='<div class="pay-row"><span>Declaracion Jurada</span><b>'+esc(r.id)+'</b></div><div class="pay-row"><span>N suministro</span><b>'+esc(r.suministro)+'</b></div><div class="pay-row"><span>N instalacion</span><b>'+esc(r.instalacion)+'</b></div><div class="pay-row"><span>Empresa instaladora</span><b>'+esc(r.empresa)+'</b></div><div class="pay-row"><span>Resultado IA</span><b>'+esc(r.resultado)+'</b></div><div class="pay-row"><span>Estado</span><b>'+esc(r.estado)+'</b></div>';
    const docs=$('#valChecklist');if(docs)docs.innerHTML=['DJ hoja 1','DJ hoja 2','Foto fachada','Foto gabinete','Foto ambiente 1','Foto ambiente 2','Foto artefacto 1','Foto artefacto 2'].map(function(d,i){const warn=(r.fotos==='5/6'&&i===4);return '<div class="pay-doc"><i>'+(warn?'!':'✓')+'</i><b>'+d+'</b><small>'+(warn?'Faltante / observar':'Validado')+'</small><em></em></div>'}).join('');
    const tags=$('#valAdminTags');if(tags)tags.innerHTML=['Datos completos','Datos incorrectos','Foto borrosa','Foto editada','Foto faltante','Caso subsanado'].map(function(t){const active=(t==='Foto borrosa'&&r.alertas.some(a=>/borrosa/i.test(a)))||(t==='Foto faltante'&&r.fotos==='5/6')||(t==='Caso subsanado'&&r.estado==='Subsanada')||(t==='Datos incorrectos'&&r.alertas.some(a=>/Direccion/i.test(a)));return '<span class="'+(active?'hu-pill warn':'')+'">'+t+'</span>'}).join('');
    const trace=$('#valTraceBox');if(trace)trace.innerHTML=[['Carga DJ','BonoGas 2.0','done'],['Analisis IA','Datos y fotos','done'],['Alerta / resultado',r.resultado,r.alertas.length?'pending':'done'],['Revision humana','Previo a liquidacion','pending']].map(function(t){return '<div class="trace-step '+(t[2]==='pending'?'pending':'')+'"><div class="trace-dot">'+(t[2]==='pending'?'○':'✓')+'</div><b>'+esc(t[0])+'</b><span>'+esc(t[1])+'</span></div>'}).join('');
    if(toast!==false&&typeof showToast==='function')showToast('DJ seleccionada: '+r.id);
  };
  ...
  window.renderSolicitudes=function(){
    ...
    if(body)body.innerHTML=filtered.map(function(s){
  ...
  function rebindFilters(){
    ...
    ['#valSearch','#valEmpresaFilter','#valEstadoFilter','#valFechaFilter','#valDistritoFilter'].forEach(function(id){const el=$(id);if(el&&!el.dataset.huBound){el.dataset.huBound='1';el.addEventListener('input',window.renderValidaciones);el.addEventListener('change',window.renderValidaciones);}});
  }

  function openAiFlow(){
    if(typeof openModal==='function')openModal('aiAssistantModal');
    const robot=$('#aiRobotBtn');
    if(robot)robot.click();
    if(typeof showToast==='function')showToast('Flujo IA de DJ abierto');
  }

  function clearBonogasProjectDetails(){
    const details=$('#details');
    if(details)details.innerHTML='';
  }

  function patchBonogasRightPanelDetails(){
    ['renderInfo','updateRightPanelForObjectSelection'].forEach(function(fnName){
      const orig=window[fnName];
      if(typeof orig!=='function'||orig.bonogasDetailsPatched)return;
      window[fnName]=function(){
        const result=orig.apply(this,arguments);
        if($('.main')&&$('.main').classList.contains('bonogas-satcontrol-mode'))clearBonogasProjectDetails();
        return result;
      };
      window[fnName].bonogasDetailsPatched=true;
    });
  }

  function initHuAlignment(){
    captureProjectSatOriginal();
    patchBonogasRightPanelDetails();
    ...
    rebindFilters();
    document.addEventListener('click',function(e){
      if(e.target.closest('[data-open-ai-flow]'))openAiFlow();
    });
...
(function(){
  ...
  function renderTabla(){
    ...
    if(!tb) return;
    tb.innerHTML = USUARIOS.map(function(u,i){
