...
function openEnhancedProjectModal(id){
  ...
  const entity=groupingMode?'agrupación':'proyecto';
  qs('#modalTitle').textContent=(isEdit?'Modificar ':'Crear ')+entity;
  qs('#projectModal .modal-head p').textContent=isEdit
    ?'Actualice los datos, el equipo, el área y los beneficiarios asociados '+(groupingMode?'a la agrupación.':'al proyecto.')
    :'Registre la información inicial '+(groupingMode?'de la nueva agrupación.':'del nuevo proyecto.');
...
qs('#projectForm').onsubmit=function(e){
  ...
  selectedId=obj.id;closeModal('projectModal');renderAll();if(qs('.main')?.classList.contains('project-list-mode'))renderProjectListTable();showToast((qs('.main.ahorro-gnv-mode')?'Agrupación':'Proyecto')+' guardado con '+rows.length+' beneficiario(s)');
};
