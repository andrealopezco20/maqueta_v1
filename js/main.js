...
window.__MODULE_PATHS = {
  ...
  'masificacion-satcontrol-source': 'modulos/masificacion/masificacion_satcontrol.html',
...
function clearProyectosBeneficiaryOverlays(){if(!leafletMap)return;[beneficiaryLayer,morosityLayer].forEach(layer=>{if(layer&&leafletMap.hasLayer(layer))leafletMap.removeLayer(layer);});}
...
async function loadIntegratedModuleSources(){
  ...
  if(masFrame&&getModuleSource('masificacion-satcontrol-source')){
    const masifBootCache='revert-v33:SATCONTROL';
...
(function(){
  ...
  function ensureBonogasMapLayer(){
    ...
    const rows=getBonogasFilteredRows().filter(function(r){
      return Number.isFinite(Number(r.lat))&&Number.isFinite(Number(r.lng))&&bonogasRecordMatchesLayerStates(r);
    });
